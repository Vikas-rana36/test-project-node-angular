...
export class CategoryComponent implements OnInit {
  ...
  _fetchListing(filter:any=""){
    ...
    if(filter){
      apiURL = '/category/listing?search="search"&page='+this.currentPage;
    }else{
      apiURL = '/category/listing?page='+this.currentPage;
    }
  ...
  onSubmit(){
    ...
    this.utilsService.processPostRequest('/category/add',this.categoryForm.value).pipe(takeUntil(this.destroy$)).subscribe((response:any) => {
  ...
  onBtnClicked(){
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.utilsService.processPostRequest('/category/deleteSelected',{id:catIds}).pipe(takeUntil(this.destroy$)).subscribe((response:any) => {
          if(response){
  ...
  delete(record:any){
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.utilsService.showPageLoader(environment['MESSAGES']['SAVING-INFO']);//show page loader
        this.utilsService.processPostRequest('/category/delete',{id:record._id}).pipe(takeUntil(this.destroy$)).subscribe((response:any) => {
          if(response){
            this.utilsService.onSuccess(environment.MESSAGES['SUCCESSFULLY-DELETED']); 
  ...
  onChaneStatus(event:any, row:any) {
    ...
    this.utilsService.showPageLoader(environment['MESSAGES']['SAVING-INFO']);//show page loader
        this.utilsService.processPostRequest('/category/changeStatus',{id:row._id, status:row.is_active?false:true}).pipe(takeUntil(this.destroy$)).subscribe((response:any) => {
          if(response){
